   ...
   <el-row class='navRow'>
      <el-col :span="22" >
        <el-breadcrumb separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item>服务站结算</el-breadcrumb-item>
          <el-breadcrumb-item>{{this.$route.query.typeLab}}</el-breadcrumb-item>
        </el-breadcrumb>
      </el-col>

      <el-col :span="2"> 
        <router-link :to="{path:'/main/10-3',query:{pageIndex:this.$route.query.pageIndex}}">
    ...
    <el-table  ref="multipleTable" :data="tableData3" size='mini'>
      ...
      <el-table-column  prop="checkpointName" label="服务站名称" width="240" show-overflow-tooltip > <template slot-scope="scope">{{ scope.row.checkpointName }}</template>   </el-table-column>
      ...
      <el-table-column  prop="inspectName" label="受检人姓名" width="150" > <template slot-scope="scope">{{ scope.row.inspectName }}</template>   </el-table-column>
      <el-table-column prop="numMoney"   label="结算金额(元)" width="150" >  <template slot-scope="scope">{{ (scope.row. numMoney/100).toFixed(2) }}</template></el-table-column>
...
export default {
  ...
  data () {
   ...
   methods: {
      searchList(id){
         this.$http.get(this.baseUrl.vip+'back/accounts/selectCheckAccountsById',
        {
          params: 
          {
            id:id

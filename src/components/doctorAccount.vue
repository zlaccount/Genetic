  ...
  <div class="container">
    ...
    <el-row class='navRow'>
      ...
      <el-col :span="19">
         ...
         <span>搜索</span>
        <el-input v-model="searchPram.userName" placeholder="请输入医生姓名" size="mini" ></el-input>
        <!-- <span>结算时间</span> -->
        <el-date-picker
    ...
    <el-table  ref="multipleTable" :data="tableData3" size='mini'>
      ...
      <el-table-column  label="操作" width="180" >
        <template slot-scope="scope">
          <router-link :to="{path:'/main/10-1/selectDoctorAcountByMonth',query:{row:scope.row,typeLab:'查看',type:true,pageIndex:searchPram.currentPage}}">
              <!-- <el-button type="primary" icon="el-icon-search" circle size="mini"></el-button> -->
              <img src="../assets/look_over.png"  title="查看" alt="">
          </router-link>
        </template>
      </el-table-column>
      <el-table-column  prop="userName" label="姓名" width="240" > <template slot-scope="scope">{{scope.row.userName}}</template>  </el-table-column>
      <el-table-column  prop="accountsDate" label="结算时间" width="240" > <template slot-scope="scope">{{scope.row.accountsDate}}</template> </el-table-column>
      <el-table-column  prop="orderNum"  label="接单数/个"  >  <template slot-scope="scope">{{ scope.row.orderNum   }}</template>  </el-table-column>
      <el-table-column prop="closeMoney"   label="总金额(元)"  >  <template slot-scope="scope">{{(scope.row.closeMoney/100).toFixed(2) }}</template></el-table-column>
      <!-- <el-table-column  prop="identity" label="检测站提成金"   width="300" show-overflow-tooltip> <template slot-scope="scope">{{ scope.row.identity }}</template>   </el-table-column> -->
...
export default {
  ...
  methods: {
    searchList(searchPram, pageIndex) {
      this.$http
        .get(this.baseUrl.vip + "back/accounts/DoctorAccounts", {
          params: {

  ...
  <div class="container">
    ...
    <el-row class='navRow'>
      ...
      <el-col :span="19">
         ...
         <span>搜索</span>
        <el-input v-model="searchPram.userName" placeholder="请输入检测站名称" size="mini" ></el-input>
        <el-date-picker
    ...
    <el-table  ref="multipleTable" :data="tableData3" size='mini'>
      ...
      <el-table-column  label="操作" width="180" >
        <template slot-scope="scope">
          <router-link :to="{path:'/main/10-3/stationAccountDetail',query:{row:scope.row,typeLab:'查看',type:true,pageIndex:searchPram.currentPage}}">
              <!-- <el-button type="primary" icon="el-icon-search" circle size="mini"></el-button> -->
              <img src="../assets/look_over.png" title="查看" alt="">
          </router-link>
        </template>
      </el-table-column>
      <el-table-column  prop="orderId" label="服务站名称" width="240" show-overflow-tooltip><template slot-scope="scope">{{scope.row.userName}} </template>  </el-table-column>
      ...
      <el-table-column  prop="inspectName" label="订单数/个"><template slot-scope="scope">{{ scope.row.orderNum   }}</template>  </el-table-column>
...
export default {
  ...
  data () {
    return {
      searchPram: {currentPage:'',options:'',accountTime:"",userName:""},
      totalNum:'',
      tableData3:'',
      multipleSelection: [],
    }
   ...
   methods: {
      searchList(searchPram,pageIndex){
         this.$http.get(this.baseUrl.vip+'back/accounts/checkAccounts',
        {
          params: 
          {
